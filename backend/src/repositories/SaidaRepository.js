...
class SaidaRepository {
    ...
    async create(saidaData) {
        ...
        try {
            ...
            if (quantidade > quantidadeDisponivel) {
                throw new Error(`Quantidade solicitada (${quantidade}) excede a disponível no lote (${quantidadeDisponivel}).`);
            }
            ...
            await connection.query(
                ...
                [novaQuantidade, id_lote]
            );

            // Se a quantidade chegou a zero, remove do estoque
            if (novaQuantidade === 0) {
                await connection.query(
                    'DELETE FROM estoque WHERE id_lote = ?',
                    [id_lote]
                );
            }
    ...
    async delete(id) {
        const [result] = await pool.query('DELETE FROM saidas WHERE id_saida = ?', [id]);
        return result.affectedRows;
    }
}
